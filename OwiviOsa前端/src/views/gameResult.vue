        ...
        <n-gi>
            <n-card>
                <n-scrollbar style="max-height: 400px">
                    <n-timeline>
                        <n-timeline-item v-for="item in gameResult.script.split('\n')" type="success" :content="item"/>
                    </n-timeline>
                </n-scrollbar>
            </n-card>
        </n-gi>
...
let team0: ComputedRef = computed(() => {
    return gameResult.value.teams_info[0].wing_cross + gameResult.value.teams_info[0].under_cutting + gameResult.value.teams_info[0].pull_back + gameResult.value.teams_info[0].middle_attack + gameResult.value.teams_info[0].counter_attack;
});
let team0_success: ComputedRef = computed(() => {
    return gameResult.value.teams_info[0].wing_cross_success + gameResult.value.teams_info[0].under_cutting_success + gameResult.value.teams_info[0].pull_back_success + gameResult.value.teams_info[0].middle_attack_success + gameResult.value.teams_info[0].counter_attack_success;
});
let team1: ComputedRef = computed(() => {
    return gameResult.value.teams_info[1].wing_cross + gameResult.value.teams_info[1].under_cutting + gameResult.value.teams_info[1].pull_back + gameResult.value.teams_info[1].middle_attack + gameResult.value.teams_info[1].counter_attack;
});
let team1_success: ComputedRef = computed(() => {
    return gameResult.value.teams_info[1].wing_cross_success + gameResult.value.teams_info[1].under_cutting_success + gameResult.value.teams_info[1].pull_back_success + gameResult.value.teams_info[1].middle_attack_success + gameResult.value.teams_info[1].counter_attack_success;
});

getGameByIdAPI({ game_id: 1 }).then(response => {
